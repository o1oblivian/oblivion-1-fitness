...
export function ModalsLayer({ s }: Props) {
  ...
  // ── Hardware back button / popstate interception ──
  ...
  useEffect(() => {
    if (anyModalOpen && !prevOpenRef.current) {
      window.history.pushState({ modalLayer: true }, '');
    }
    prevOpenRef.current = anyModalOpen;
  }, [anyModalOpen]);
  ...
  useEffect(() => {
    const onPop = () => {
      ...
      if (s.isVaultViewerOpen) {
        s.setIsVaultViewerOpen(false);
        return;
      }
      if (s.isGlobalSearchOpen) {
        s.setIsGlobalSearchOpen(false);
        return;
      }
      if (s.isBuddyRadarOpen) {
        s.setIsBuddyRadarOpen(false);
        return;
      }
      if (s.isGymNetworkOpen) {
        s.setIsGymNetworkOpen(false);
        return;
      }
      if (s.isCommunityHubOpen) {
        s.setIsCommunityHubOpen(false);
        return;
      }
      if (s.isCoachShowroomOpen) {
        s.setIsCoachShowroomOpen(false);
        return;
      }
      if (s.isOwnProfileOpen) {
        s.setIsOwnProfileOpen(false);
        return;
      }
      if (s.isAIInsightsOpen) {
        s.setIsAIInsightsOpen(false);
        return;
      }
      if (s.isPayPlanOpen) {
      ...
      if (s.isTravelPassOpen) {
        s.setIsTravelPassOpen(false);
        return;
      }
      if (s.isShareModalOpen) {
        s.setIsShareModalOpen(false);
        return;
      }
      if (s.showReminderManager) {
        s.setShowReminderManager(false);
        return;
      }
      if (s.isExportHelpOpen) {
        s.setIsExportHelpOpen(false);
        return;
      }
      if (s.dialConfig.isOpen) {
        s.setDialConfig((p) => ({ ...p, isOpen: false }));
        return;
      }
      if (s.customFoodQuery !== null) {
        s.setCustomFoodQuery(null);
        return;
      }
      if (s.foodModalMeal) {
        s.setFoodModalMeal(null);
        return;
      }
      if (s.isCommitModalOpen) {
        s.setIsCommitModalOpen(false);
        return;
      }
      if (s.isAutoPilotOpen) {
        s.setIsAutoPilotOpen(false);
        return;
      }
      if (s.isCycleSyncOpen) {
        s.setIsCycleSyncOpen(false);
        return;
      }
      if (s.isRoutineSwapperOpen) {
        s.setIsRoutineSwapperOpen(false);
        return;
      }
      if (s.isScheduleModalOpen) {
