  ...
  return (
    <button
      ...
      className={`relative inline-flex shrink-0 cursor-pointer items-center rounded-full p-[2px] transition-colors duration-200 ease-in-out focus:outline-none select-none ${
        ...
        checked ? 'bg-[#C4121A] dark:bg-[#D91F28]' : 'bg-zinc-300 dark:bg-zinc-700'
      }`}
    >
      <span
        style={{
  ...
  return (
    ...
    >
      <div className="flex-1 min-w-0 pr-2">
        <span className={`text-sm font-medium block leading-snug ${destructive ? 'text-[#C4121A] dark:text-[#D91F28] font-semibold' : 'text-zinc-900 dark:text-zinc-100'}`}>
          {label}
        </span>
        {sublabel && (
...
export function NavPill({
  ...
  return (
    ...
    >
      <span className={`text-sm font-medium ${destructive ? 'text-[#C4121A] dark:text-[#D91F28] font-semibold' : 'text-zinc-900 dark:text-white'}`}>
        {label}
      </span>
      <div className="flex items-center gap-2 shrink-0">
