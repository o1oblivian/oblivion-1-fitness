...
export function SettingsPage({
  ...
  const { getProfile } = useAuthStorage();
  ...
  return (
    <div className="fixed inset-0 z-[200] bg-[#F4F4F5] dark:bg-[#09090B] overflow-hidden font-sans select-none flex flex-col">
      {/* Top Header Bar */}
      <div className="sticky top-0 z-30 bg-white/90 dark:bg-[#09090B]/90 backdrop-blur-md px-3.5 py-2 border-b border-zinc-200/80 dark:border-zinc-800/80 flex items-center justify-between shrink-0 h-12">
        <button
          type="button"
          onClick={onClose}
          className="w-7 h-7 rounded-full flex items-center justify-center hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-900 dark:text-white cursor-pointer transition-colors"
        >
        ...
        <button
          type="button"
          onClick={onClose}
          className="text-xs font-semibold text-[#DC2626] hover:text-red-600 cursor-pointer px-1 py-0.5"
        >
          Done
        </button>
      </div>
