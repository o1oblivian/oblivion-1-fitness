...
async function startServer() {
  ...
  app.use("/videos", import_express.default.static(import_path.default.join(process.cwd(), "public", "videos")));
  app.get("/api/download-video/:filename", (req, res) => {
    const safeName = import_path.default.basename(req.params.filename);
    const videoPath = import_path.default.join(process.cwd(), "public", "videos", safeName);
  ...
  app.get(["/api/download-upload-cert", "/upload_certificate.pem"], (_req, res) => {
    const certPath = import_path.default.join(process.cwd(), "public", "upload_certificate.pem");
...
Return ONLY a valid JSON object matching this schema:
{
  ...
  "summary": "1:57 elapsed, 0.02 km, 3 calories burned at 0.8 km/h"
      ...
      try {
        ...
        let durationMinutes = typeof parsed.durationMinutes === "number" ? Math.round(parsed.durationMinutes * 10) / 10 : parsed.durationSeconds ? Math.round(parsed.durationSeconds / 60 * 10) / 10 : 0;
        let caloriesBurned = typeof parsed.caloriesBurned === "number" ? Math.round(parsed.caloriesBurned) : 0;
        let distanceKm = typeof parsed.distanceKm === "number" ? Math.round(parsed.distanceKm * 100) / 100 : void 0;
        const speedKmh = typeof parsed.speedKmh === "number" ? Math.round(parsed.speedKmh * 10) / 10 : void 0;
        let stepsCount = typeof parsed.stepsCount === "number" ? Math.round(parsed.stepsCount) : void 0;
        if (stepsCount && stepsCount > 0) {
          if (!caloriesBurned || caloriesBurned === 0) {
            caloriesBurned = Math.round(stepsCount * 0.045);
          }
          if (!distanceKm || distanceKm === 0) {
            distanceKm = Math.round(stepsCount * 762e-6 * 100) / 100;
          }
        ...
        return res.json({
          ...
          result: {
            ...
            detectedBrand: parsed.detectedBrand || void 0,
            caloriesBurned,
            durationMinutes,
...
- Daily Steps: ${intake.currentDailySteps || 8e3} (Goal: ${intake.dailyStepGoal || 1e4})
  ...
  "coachNotes": "Specific safety and execution directives"
      ...
      const fallbackRecommendation = {
        ...
        recoveryProtocol: {
          ...
          stepsTarget: intake.dailyStepGoal || 8e3
        },
...
<body>
  ...
  app.get("/api/download-video/:filename", (req, res) => {
    ...
    const candidatePaths = [
      import_path.default.join(process.cwd(), "public", "videos", filename),
      import_path.default.join(process.cwd(), "dist", "videos", filename)
    ];
  ...
  app.get("/api/download-photo/:filename", (req, res) => {
    const filename = import_path.default.basename(req.params.filename);
    const candidatePaths = [
      import_path.default.join(process.cwd(), "public", "live_photos", filename),
      import_path.default.join(process.cwd(), "dist", "live_photos", filename),
