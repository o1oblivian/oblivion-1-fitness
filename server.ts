...
Return ONLY valid JSON matching this schema:
{
  ...
  app.post('/api/stripe-checkout', standardApiLimiter, async (req, res) => {
    try {
      ...
      const effectiveWebOrigin = (clientOrigin && clientOrigin.startsWith('http'))
        ? clientOrigin
      ...
      if (isMobile) {
        ...
        finalSuccessUrl = successUrl
          ...
          : `${effectiveWebOrigin}?payment=success&tier=${encodeURIComponent(planId || 'premium')}&session_id=${sessionUrlParam}`;
  ...
  // Stripe In-App Mobile & Web Payment Return Bridge
...
<body>
  ...
  <script>
    ...
    const returnPath = status === 'success'
      ? `/?payment=success&tier=${encodeURIComponent(String(tier))}&session_id=${encodeURIComponent(String(session_id || ''))}`
      : '/?payment=cancel';
